...
public interface ITranslationReleaseRepository : IRepository <TranslationRelease>
{
    ...
    //Task<(List<ReleaseHistoryItemDto> Items, int TotalCount)> GetReleaseHistoryAsync(Guid projectId, int pageNumber, int pageSize, CancellationToken cancellationToken);
