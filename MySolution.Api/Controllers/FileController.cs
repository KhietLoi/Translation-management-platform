...
public class FileController(IMediator mediator) : Controller
{
    ...
    {
        var response = await mediator.Send(new UploadFileCommand(new UploadFileResquest { File = file }),
            cancellationToken);
    ...
    {
        var response = await mediator.Send(new DeleteFileCommand(new DeleteFileRequest { FileName = req.FileName }),
            cancellationToken);
